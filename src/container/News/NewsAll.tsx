...
export const NewsAll: React.FC<NewsAllProps> = ({
  selectKey,
  setSelectKey,
  menu,
  dataArr,
        ...
        {menu?.map((listType) => (
          ...
          >
            <ul className="bg-slate-50 p-4 sm:px-8 sm:pt-6 sm:pb-8 lg:p-4 xl:px-8 xl:pt-6 xl:pb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4 text-sm leading-6">
              {dataArr
                ?.filter((list) => listType?.id === list?.newsTypeId)
                ?.map((list) => (
                      ...
                      <div className="flex items-center">
                        <img
                          ...
                          src={`https://imagedelivery.net/QZ6TuL-3r02W7wQjQrv5DA/${
                            list?.img || "4500f404-dbac-40f3-6696-ae768a38e800"
                          }/150`}
            ...
            {checkPage
              ?.filter((list) => listType?.id === list?.type)
              ?.map((list) => (
                <div key={list.type} className="text-center">
                  <button
                    ...
                    className={`text-yellow-800 hover:text-yellow-900 text-sm leading-6 font-medium py-2 px-3 rounded-lg  ${
                      list.page === list.totalPages ? "hidden" : ""
                    }`}
